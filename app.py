...
# ==========================================
st.set_page_config(page_title="HKDSE Statistical Report Data Converter | HKDSE學校統計報告 數據轉換工具", page_icon="🔁", layout="wide", initial_sidebar_state="collapsed")
                ...
                try:
                    ...
                    if df_total.empty:
                        ...
                        if attendance_ys is not None and attendance_ds is not None:
                            ...
                            st.subheader("📈 貴校與日校比較 - 柱形圖 | Comparison of Your school and Day schools - Bar chart")
            ...
            with st.spinner("系統正在處理檔案，請稍候... | Processing file, please wait..."):
                try:
                    ...
                    if df_item.empty:
                        ...
                        if st.button("📌📌 打開自定義項目分析 app", type="primary", use_container_width=True, key="btn_item_app"):
                            st.switch_page("pages/1_custom_item_app.py")
                        st.subheader("📋 數據概覽 | Data Preview")
                        df_item_display = df_item.drop(columns=["初始序列"]) if "初始序列" in df_item.columns else df_item
            ...
            with st.spinner("系統正在處理檔案，請稍候... | Processing file, please wait..."):
                try:
                    ...
                    if df_mcq.empty:
                        ...
                        if st.button("🎯🎯 打開自定義 MCQ 分析 app", type="primary", use_container_width=True, key="btn_mcq_app"):
                            st.switch_page("pages/2_custom_mcq_app.py")
                        st.subheader("📋 數據概覽 | Data Preview")
                        df_mcq_display = df_mcq.drop(columns=["初始序列"]) if "初始序列" in df_mcq.columns else df_mcq
